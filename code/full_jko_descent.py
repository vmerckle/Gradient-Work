...
class jko_descent:
    ...
    def step(self):
        ...
        for i in range(self.interiter):
            ...
            self.p = self.proxf(self.K(b)) # as algo
            #print("before", f"a: {a.sum()}, b:{b.sum()}, p:{self.p.sum()}")
    ...
    def prox_f_pytorch(self, q_in):
        #qsum = q_in.sum()
        #if qsum > 1.01:
        #print("warning proxf in vector..sum=", qsum)
            #assert qsum < 1.2
        X = torch.tensor(self.X, dtype=self.dtype, device=self.device)
        Y = torch.tensor(self.Y, dtype=self.dtype, device=self.device)
        p = torch.tensor(q_in, dtype=self.dtype, device=self.device, requires_grad=True)
        ...
        for i in range(maxit):
            ...
            with torch.no_grad():
                ...
                newp = torch.clamp(newp, min=1e-7)
                #newp = newp/newp.sum()
                p.copy_(newp)
    ...
    def f(self, p):
        p = p.flatten()
        print(p.shape, self.psigns.shape)
        activ = (self.X @ self.grid.T) > 0 # (n, N)
        effneurons = p * self.grid.T # (d,N) 
...
def getKernel(grid, gamma):
    ...
    # Gibbs = (Gibbs > 1e-5) * Gibbs # this doesn't solve any problem
    #print(Gibbs)

    def aux(p):  # Gibbs Kernel applied to a vector p 
        #print(Gibbs.shape, p.shape)
