...
class NiceAnim:
    ...
    def update(self, frame):
        ...
        xl, yl, mks = np.zeros(m), np.zeros(m), []
        sizes, colors = lsize, []
        for i in range(m):
            w1, w2 = ly1.T[i]
            alpha, = ly2[i]
            ...
            if i == 88:
                #print(f"slow dude w=({w1:.2f}, {w2:.2f})*{alpha:.2f}. acti={lact[i]:.2f}")
                colors.append("cyan")
            elif i == 58:
                #print(f"fast dude w=({w1:.2f}, {w2:.2f})*{alpha:.2f}. acti={lact[i]:.2f}")
...
def NNtoIter(Xt, Yt, allX, ly1, ly2):
...
def simplerun(opti, Nstep):
    ...
    for num in range(Nstep-1):
        ...
        if num%1 == 0:
            #print(nly2)
            #print("ly1 dist", np.linalg.norm(ly1-nly1)) #indeed, it's 0
            print("ly2 dist", np.linalg.norm(ly2-nly2))
            lly1.append(nly1)
            lly2.append(nly2)
...
def simplecalcs(X, Y, lly1, lly2, rng):
    ...
    data = {"Xb": Xb, "Y": Y, "X": X, "Xout": allXb, "iterdata": iterdata}
...
if __name__ == '__main__':
    ...
    parser.add_argument("--jkogamma", default=1, type=float)
    parser.add_argument("--jkotau", default=1, type=float)
    ...
    Xb = np.array([0.2, 0.5, 1])[:, None]
    X, Y = add_bias(Xb), np.array([0.1, 0.4, 0.9])[:, None]
    ...
    if args.algo == "torch":
        ...
        opti = jko_descent(interiter=args.jkosteps, gamma=args.jkogamma, tau=args.jkotau)
        opti.load(X, Y, ly1, ly2, lr, beta)
    ...
    if args.movie:
        ...
        print(f"saved as {code}_movie.mp4")
    else:
        plt.show()
